...
abstract class EloquentRepository implements ModelInterface
{
    ...
    /**
     * {@inheritDoc}
     * Метод получения моеделей, где $field == $value и сортировка
     * @param string $field
     * @param $value
     * @return mixed
     */
    public function findAllByOrderBy(string $field, string $value, string $parameterOne, string $parameterTwo)
    {
        return $this->getModel()->where($field, $value)->orderBy($parameterOne, $parameterTwo)->get();
    }
